...
KeySnail.UserScript = {
    ...
    jsFileLoader: function (aScriptPath, aPreserve) {
        ...
        if (this.parent.windowType === "mail:3pane" && aPreserve)
            this.preserveCode(code);
        Function("with (KeySnail.modules) {" + code + " }")();
    },

    // ==== user configuration file name ====
    // at first .keysnail.tb.js is used. When the file not found,
    // then the _keysnail.tb.js is used instead. (for the Windows user)
    defaultInitFileNames: [".keysnail.tb.js", "_keysnail.tb.js"],
    initFilePath: null,
    ...
    load: function () {
        ...
        if (loadStatus == -1)
        {
            ...
            if (this.parent.windowType == "mail:3pane")
            {
